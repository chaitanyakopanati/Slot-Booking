...
import {getSuppliersData} from '../helperSupplier/ModelTypeSupplier'
  ...
  const deletePackagesCategoriesData = (ID: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        LoderActions(true)
        try {
          let payload = await Supplierservice.deleteSuppliers(ID)
        ...
        <table className='d-none d-md-table table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3 table-rounded border table-striped'>
          ...
          <thead>
            <tr className='fw-bolder text-muted  bg-dark'>
              ...
              <th className='min-w-150px'>Gst No</th>
          ...
          <tbody>
            {getData.length > 0 ? (
              getData?.map((row: getSuppliersData, index: number) => {
                return (
                  <tr key={index}>
                    {/* begin:: Index No */}
                    <td>
                    ...
                    <td className='text-dark fw-bold  fs-6'>{row?.gstNo ? row?.gstNo : '-'}</td>
                    {/* end:: Etr(Hours) Input */}
