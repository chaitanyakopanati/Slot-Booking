...
import { getFaultsData } from '../../helperFaults/ModelFaultsType'

const FaultsTable = () => {
  const {setItemIdForUpdate, getData,pageNo,pageSize, setViewIdForUpdate,DataGetAllType,fetchAllFault,searchText} =
    ListPageData()
    ...
    <>
        <div className='table-responsive'>
          ...
          <table className='table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3 table-rounded border table-striped'>
            ...
            {/* begin::Table body */}
            <tbody>
              {getData?.map((FautlData: getFaultsData, index: number) => {
                return (
                  <tr key={index}>
                    <td>
